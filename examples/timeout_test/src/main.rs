...
lazy_static! {
    ...
    static ref INSTALLED_FLOWS: Mutex<HashMap<FiveTuple, FlowEntry>> = Mutex::new(HashMap::new());
}
...
fn expire_flows_now() {
    ...
    let mut installed = INSTALLED_FLOWS.lock().unwrap();
    let now = Instant::now();
    ...
    while let Some(entry) = queue.front() {
        ...
        // Also remove from the installed flows map
        installed.remove(&expired.tuple);
    }
}
        ...
        .add_dispatcher(flow_dispatcher.clone(), |event: FlowEvent| {
            ...
            match event {
                FlowEvent::TlsSeen { tuple, .. } => {
                    ...
                    {
                        let installed = INSTALLED_FLOWS.lock().unwrap();
                        if installed.contains_key(&tuple) || installed.len() >= NUM_FLOWS {
                            return;
                        }
                    }
                    ...
                    if let Some(ports) = maybe_ports {
                        match install_drop_flow(ports.clone(), &tuple) {
                            Ok(raw_flows) => {
                                ...
                                FLOW_QUEUE.lock().unwrap().push_back(entry.clone());
                                INSTALLED_FLOWS.lock().unwrap().insert(tuple.clone(), entry);
                            }
